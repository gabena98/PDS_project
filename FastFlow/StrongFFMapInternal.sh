...
workers=(1 2 4 8 16 20 32 48 64)

# Output directory for the statistics
stats_output_dir="./strong_map_stats_internal"
...
for first_input in "${dim[@]}"; do
  ...
  for second_input in "${workers[@]}"; do
    echo "Executing FinalFarmFF with inputs: N = $first_input, workers = $second_input"
    
    elapsed_times=()
    
    # Run the program 5 times for the current configuration
    for i in {1..5}; do
      output=$(srun -N 1 FinalMapFF $first_input $second_input)
